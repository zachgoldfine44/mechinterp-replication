...
milestone-level state see `PROGRESS.md`. **Never rewrite past entries
here.**

Format: each entry starts with an ISO date, the commit short-hash if
known, and a one-line summary, followed by optional bullets.
...
- Tier-2 refactors specific to emotions analysis (aggregation, ActivationCache
  wiring for emotion-concept experiments — commits `547c454`, `4087d90`).
